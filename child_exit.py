...
while True:  #sorry god of computers for that
	...
	faces    = face_cascade.detectMultiScale(gray, scaleFactor=1.025, minNeighbors=4, minSize=(80,80))
	out = img.copy()
	...
	for(x,y,w,h) in faces:	
		...
		if conf < 50:
			...
			json = req.json()
			name = json['name']
			req = requests.get(API_BASE + f'/child/inschool/{str(ids)}/', auth = (user, passw))
